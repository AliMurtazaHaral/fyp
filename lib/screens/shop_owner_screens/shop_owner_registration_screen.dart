...
class ShopOwnerRegistrationScreen extends StatefulWidget {
  const ShopOwnerRegistrationScreen({Key? key}) : super(key: key);

  @override
  State<ShopOwnerRegistrationScreen> createState() => _ShopOwnerRegistrationScreenState();
}

class _ShopOwnerRegistrationScreenState extends State<ShopOwnerRegistrationScreen> with TickerProviderStateMixin {
  ...
  final shopRegistrationNumberEditingController = TextEditingController();
  ...
  Widget build(BuildContext context) {
    ...
    //City Field
    final shopRegistrationNumberField = TextFormField(
      autofocus: false,
      controller: shopRegistrationNumberEditingController,
      keyboardType: TextInputType.text,
      validator: (value) {
        if (value!.isEmpty) {
      ...
      onSaved: (value) {
        shopRegistrationNumberEditingController.text = value!;
      },
      textInputAction: TextInputAction.next,
      decoration: const InputDecoration(
        fillColor: Colors.white,
          ...
          child: MaterialButton(
            ...
            onPressed: () async {
              ...
              if (result == null) {
                ...
                getShopUrl = FirebaseStorage.instance.ref().child(fileName!);
              }
            },
            child: const Text(
              "Upload Shop Image",
    ...
    //signup button
    final signUpButton =
    PrimaryButton.primaryButton(context, "Sign Up", onPressed: () {
  ...
  postDetailsToFirestore() async {
    ...
    userModel.profession = "Shop Owner";
    userModel.profileImageReference = getUrl.fullPath.toString();
    userModel.shopName = shopNameEditingController.text;
    userModel.shopImageReference = getShopUrl.fullPath.toString();
    userModel.rating = "0";
    await firebaseFirestore
        .collection("users")
        .doc(user?.uid)
        .set(userModel.toBecomeShopOwnerRegistration());
